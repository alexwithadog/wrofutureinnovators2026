...
Notes:
  ...
  - Camera is plain V4L2 + OpenCV. No GStreamer / nvargus.
  - Mic is stereo on this flash; we capture stereo and take left channel.
  - Memory wipes on language switch so context doesn't leak between languages.
"""
...
# Keep the full 16:9 FOV instead of cropping to a square. Bigger view of the room.
CAMERA_PROCESS_SIZE = (1280, 720)
# YOLOE inference resolution. Higher = better detection at distance, slower.
# 320 was fast but cropped objects miss. 480 is a good balance for far objects.
YOLOE_IMGSZ = 480
...
"""
    ...
    def _switch_language(self, new_lang: str) -> None:
        ...
        # Wipe conversation memory so the new language starts with a clean slate.
        with self.memory_lock:
            self.memory.clear()
