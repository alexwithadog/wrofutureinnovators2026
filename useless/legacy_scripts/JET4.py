...
Startup behavior:
  - Terminal-based language picker. Type 1/2/3 to choose English/French/Spanish.
  - Acknowledgment WAVs are CACHED to disk in ~/.atlas_ack_cache/
    First run: pre-renders all 18 WAVs (~30-45s).
    Subsequent runs: skip rendering if files exist (~3-5s startup).
  - Camera preview only opens if a display is available (locally or X11 forward).
"""
...
# Persistent dir for pre-rendered Piper WAVs. They never change between runs
# unless LANGUAGES dict is edited, so caching saves 30-45s on every startup.
...
CAMERA_HEIGHT = 720
CAMERA_FPS = 30
CAMERA_ROTATION = None  # cv2.ROTATE_180 if mounted upside down, etc
...
def _ensure_ack_wav(voice: str, text: str, cache_path: str) -> bool:
    """Render WAV only if cache file doesn't already exist.
    Returns True if file exists (newly rendered or already cached)."""
...
class MuseumHelmet:
    def __init__(self, initial_language: str = DEFAULT_LANGUAGE):
        ...
        # --- Pre-rendered acknowledgment WAVs per language (persistent cache) ---
