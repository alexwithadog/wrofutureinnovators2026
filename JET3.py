...
Display behavior:
  - With monitor attached: Tkinter language picker shows at startup,
    camera preview window opens.
  - Headless (SSH / no monitor): picker is skipped (defaults to English),
    no preview window. Voice commands still work to switch language.
"""
...
CAMERA_HEIGHT = 720
CAMERA_FPS = 30
CAMERA_ROTATION = None  # cv2.ROTATE_180, etc, or None
...
def _has_display() -> bool:
    """True if a display is available (DISPLAY env var or Wayland)."""
...
# --------------------------------------------------------------------------
# Language picker (Tkinter, shown only if display attached).
# --------------------------------------------------------------------------
def show_language_picker() -> str:
    """Show a Tkinter picker. Returns the chosen language key.
    If no display available or Tkinter fails, returns DEFAULT_LANGUAGE.
    """
    if not _has_display():
        print("[Picker] No display detected — defaulting to English.")
        return DEFAULT_LANGUAGE

    try:
        import tkinter as tk
    except ImportError:
        print("[Picker] Tkinter not available — defaulting to English.")
        return DEFAULT_LANGUAGE

    chosen: dict = {"value": None}

    root = tk.Tk()
    root.title("ATLAS — Choose Language")
    # Set a reasonable size and center on screen
    win_w, win_h = 420, 260
    ...
    subtitle = tk.Label(
        root,
        text="Choose a language to start:",
        font=("Helvetica", 12),
        bg="#1e1e1e",
        fg="#bbbbbb",
        pady=6,
    ...
    for lang_key, cfg in LANGUAGES.items():
        btn = tk.Button(
            button_frame,
            text=cfg["display_label"],
            font=("Helvetica", 13, "bold"),
            width=12,
            height=2,
            bg="#2d6cdf",
            fg="white",
            activebackground="#1e4ea5",
            activeforeground="white",
            relief="flat",
            command=lambda l=lang_key: pick(l),
        )
        btn.pack(side=tk.LEFT, padx=8)

    note = tk.Label(
        root,
        text="(You can change language later by saying e.g. 'switch to french')",
        font=("Helvetica", 9),
        bg="#1e1e1e",
        fg="#888888",
        pady=6,
    ...
    # If user closes the window without picking, treat as default.
    def on_close() -> None:
        chosen["value"] = DEFAULT_LANGUAGE
        root.destroy()

    root.protocol("WM_DELETE_WINDOW", on_close)
    root.mainloop()

    return chosen["value"] or DEFAULT_LANGUAGE
...
"""
    ...
    def camera_worker(self) -> None:
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                # Only render preview if a display is available.
                if self.has_display:
                    ...
                    try:
                        ...
                        # If the window subsystem fails mid-run, log once
                        # and disable further preview attempts.
                        print("[Display] cv2.imshow failed, disabling preview.")
    ...
    def _maybe_trigger_object_explanation(self, detections: list[dict]) -> None:
        ...
        # Strict-confidence triggerable detections.
        triggerable = [
            d for d in detections
            if d["name"] in TRIGGER_OBJECTS
            and d["confidence"] >= TRIGGER_CONFIDENCE_THRESHOLD
        ]
        # Lower-confidence "still seeing it" detections — keeps the hold timer
        # alive through brief confidence dips.
        still_seeing = [
            d for d in detections
            if d["name"] in TRIGGER_OBJECTS
    ...
    def start(self) -> None:
        ...
        # Greeting in the active language (already set by main()).
...
if __name__ == "__main__":
    # Show language picker BEFORE constructing helmet, so we know
    # which language to start in. Headless = default English.
